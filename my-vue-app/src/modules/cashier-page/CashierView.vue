...
const confirm =  () => {
  ...
  try {
    ...
    // Redirect to another page after successful confirmation
    router.push("/product");
...
<template>
  <div class="p-3 space-y-3">
    ...
    <!-- Empty Cart Message -->
    <div v-else class="text-center py-10 space-y-4 border rounded-md bg-gray-50">
      <p class="text-xl font-semibold text-gray-600">Your order is empty!</p>
    </div>

    <!-- Total Price Section -->
    <div v-if="cartStore.cartItems.length > 0" class="flex justify-between items-center pt-4">
      <p class="text-lg font-bold">Total: ${{ cartStore.totalCartPrice.toFixed(2) }}</p>
      <Button variant="secondary" class="px-6 py-2" @click="confirm">Confirm Order</Button>
    </div>
  </div>
</template>